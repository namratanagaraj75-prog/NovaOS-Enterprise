...
import { auth, db, isFirebaseMocked } from "../lib/firebase";
...
const FIREBASE_CONFIG_MESSAGE =
  "Firebase is not configured. Add the VITE_FIREBASE_* environment variables to enable sign-in.";
...
const normalizeRole = (role?: string | null) => {
  ...
  if (value === "HIRING_MANAGER" || value === "MANAGER")
...
const authErrorFrom = (
  error: any,
  fallback = "Authorization check failed.",
) => {
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const readAuthorizedEmployee = async (
    fbUser: FirebaseUser,
  ): Promise<AuthUser> => {
    ...
    return {
      ...
      displayName: String(
        data.displayName || data.name || fbUser.displayName || "NovaOS User",
      ),
      role,
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      ...
      if (
        ...
        errorMessage =
          "Malformed email address structure. Please verify formatting.";
      } else if (error.code === "auth/network-request-failed") {
        errorMessage =
          "Network offline or disconnected. Check internet connection parameters.";
      }
...
export const useAuth = () => {
  ...
  if (!context) {
    throw new Error(
      "useAuth must be called from within an AuthProvider scope.",
    );
  }
  return context;
};
