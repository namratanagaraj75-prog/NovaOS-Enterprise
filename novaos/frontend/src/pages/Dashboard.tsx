...
import { AlertTriangle, CheckCircle2, FileText, LogIn, Mail, UserCheck, Users, Workflow } from 'lucide-react';
...
export const Dashboard: React.FC = () => {
  const { user } = useAuth();
  const { hiringRequests, candidates, documents, emailNotifications, activities, recentAccess, loading } = useAppContext();
  const metrics = useMemo(() => ({
    total: candidates.length,
    pending: hiringRequests.filter(item => pendingStatuses.includes(item.status)).length,
    approved: candidates.filter(item => ['APPROVED','APPROVALS_COMPLETED','OFFER_GENERATED','OFFER_SENT','EMAIL_SENT','WORKFLOW_COMPLETED','EMPLOYEE_CREATED','HIRED'].includes(String(item.currentStatus||item.status))).length,
    rejected: candidates.filter(item => ['REJECTED','Rejected'].includes(String(item.currentStatus||item.status))).length,
    offersGenerated: documents.filter(item=>item.documentType==='OFFER_LETTER'&&item.status==='GENERATED').length,
    offersSent: emailNotifications.filter(item=>item.type==='OFFER'&&item.status==='SENT').length,
    hired: candidates.filter(item=>['EMPLOYEE_CREATED','HIRED','Employee Created','Hired'].includes(String(item.currentStatus||item.status))).length,
    active: hiringRequests.filter(item => !['REJECTED','WORKFLOW_COMPLETED','EMPLOYEE_CREATED'].includes(item.status)).length,
  }), [hiringRequests, candidates, documents, emailNotifications]);
  ...
  const statusChart = useMemo(() => [
    ...
    ['Approved', metrics.approved], ['Rejected', metrics.rejected], ['Offer Sent', metrics.offersSent],
  ...
  const cards = [
    ['Total Candidates',metrics.total,Users,'Canonical candidate records'], ['Pending Approvals',metrics.pending,Workflow,'Across all departments'],
    ...
    ['Offers Generated',metrics.offersGenerated,FileText,'Generated from completed approvals'], ['Offers Sent',metrics.offersSent,Mail,'Successful email delivery'],
    ['Hired',metrics.hired,UserCheck,'Employee records created'], ['Active Workflows',metrics.active,Workflow,'Not completed or terminated'],
  ] as const;
  ...
  return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 text-slate-200">
    ...
    <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6"><div className="flex items-center gap-2 border-b border-slate-800 pb-4 mb-4"><LogIn className="h-5 w-5 text-violet-400" /><h2 className="font-bold text-white">Recent Access</h2></div>
      {recentAccess.length ? <div className="grid gap-3 md:grid-cols-3">{recentAccess.map(access => <div key={access.id} className="bg-slate-950/60 border border-slate-800 rounded-xl p-4"><p className="text-sm font-semibold text-slate-200">{access.userName}</p><p className="text-[10px] uppercase tracking-wider text-violet-400 mt-1">{label(access.role)}</p><p className="text-xs text-slate-500 mt-3">{access.time}</p></div>)}</div> : <p className="py-8 text-center text-xs text-slate-500">No successful access has been recorded yet.</p>}
    </section>

    <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6"><div className="flex items-center gap-2 border-b border-slate-800 pb-4 mb-4"><FileText className="h-5 w-5 text-cyan-400" /><h2 className="font-bold text-white">Recent Hiring Activity</h2></div>
      {activities.length ? <div className="space-y-3">{activities.slice(0,3).map(item => { const request = hiringRequests.find(candidate => candidate.id === item.requestId); return <div key={item.id} className="flex items-start justify-between gap-4 bg-slate-950/60 border border-slate-800 rounded-xl p-4"><div><p className="text-sm text-slate-200"><strong>{request?.candidateName || 'Hiring request'}</strong> · {label(item.message)}</p><p className="text-xs text-slate-500 mt-1">{item.sub || `${item.actorName || 'NovaOS'} recorded this event.`}</p></div><span className="text-[9px] text-slate-500 font-mono shrink-0">{item.time}</span></div>})}</div> : <p className="py-8 text-center text-xs text-slate-500">No hiring activity has been recorded yet.</p>}
    </section>
  </motion.div>;
};
